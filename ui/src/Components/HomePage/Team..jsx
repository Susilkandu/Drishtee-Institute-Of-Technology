            ...
            >
                {expertData.map((expert, index) => (
                    <SwiperSlide key={index} className='swiper-slide'>
                        <div className="card px-5 py-2 team m-2 text-center teamCard " id={`expert${index + 1}`}>
                            <img src={expert.image} style={{ border: '5px solid lightgray' }} className="card-img-top img-size rounded-circle" alt={expert.name} />
                            <span className="w-100 fw-bold" style={{ color: 'black' }}>{expert.name}</span>
                            <small className="fw-normal text-center" style={{color:'black'}}>{expert.role}</small>
                        </div>
                    </SwiperSlide>
                ))}
            </Swiper>
