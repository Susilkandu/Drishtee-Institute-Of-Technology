            ...
            bcrypt
              ...
              .then((password) => {
                student
                  ...
                  .then(async (data) => {
                    const title = `Hello ${data.name} Your Admission has been confermed`;
                    const message = `Your Registration number is: <h1>${data.regNum}</h1><br>Your User ID is: <h3>${data.regNum}</h3><br>Your  <h3>${psw}</h3>`;
...
const updateNotice = catchAsyncError(async (req, res, next) => {
  ...
  if (!_id) {
    ...
    .then((updatedNotice) => {
      ...
      return res.status(201).json({ ackbool: 1, message: "Notice updated successfully", updatedNotice });
    })
    .catch((error) => {
      return next(new ErrorHandler(500, error));
    });
